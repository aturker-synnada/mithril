...
class Linear(Model):
    ...
    def __init__(
        ...
        if use_bias:
            self += mult(left=IOKey("input", value=input), right=weight_key)
            self += Add()(
...
class TSNECore(Model):
    ...
    def __init__(
        ...
        if exact_distances:
            square_model = Square()
            self += square_model(input=IOKey("distances", value=distances))
        ...
        self += sum_model_1(
            left=1.0, right=IOKey("pred_distances", value=pred_distances)
        )
